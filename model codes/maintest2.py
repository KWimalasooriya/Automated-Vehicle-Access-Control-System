...
while True:
            ...
            if result >= 0:
                if track_id not in counter:
                    ...
                    vehicle_text_map[track_id] = {
                        ...
                        'is_inside': True  # Mark as inside frame
                    }

            # Vehicle has left the detection area (outside frame)
            else:
                if track_id in vehicle_text_map:
                    vehicle_text_map[track_id]['is_inside'] = False  # Mark as outside the frame
    ...
    if len(vehicle_text_map) > 0:
        ...
        # If the vehicle is still inside, set the text color to green
        text_color = (0, 255, 0) if latest_vehicle['is_inside'] else (0, 0, 255)
        ...
        latest_data = {
            ...
            "text_color": text_color  # Adding text color to the response
        }

    # Draw the static blue rectangle on the frame
    draw_static_rectangle(frame)

    # Display the number plate text in green while vehicle is inside the frame
    if len(vehicle_text_map) > 0:
        latest_vehicle = vehicle_text_map[list(vehicle_text_map.keys())[-1]]
        if latest_vehicle['is_inside']:
            text_color = (0, 255, 0)  # Green color for text
        else:
            text_color = (0, 0, 255)  # Red color for text

        cv2.putText(frame, latest_vehicle['text'], (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, text_color, 2)
